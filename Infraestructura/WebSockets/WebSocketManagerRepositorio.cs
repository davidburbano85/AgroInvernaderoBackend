...
namespace invernaderoInteligenteBackend.Infraestructura.WebSockets
{
    public class WebSocketManagerRepositorio : IWebSocketManager
    {
        ...
        // NUEVO:
        // Permite que cada POST espere la respuesta correspondiente
        // del ESP32 sin leer una respuesta anterior.
        private readonly ConcurrentDictionary<
            string,
            TaskCompletionSource<string>
        > _respuestasPendientes;
        ...
        // NUEVO:
        // Prepara la espera ANTES de enviar el mensaje.
        // Así no existe la posibilidad de que el ESP32 responda
        // antes de que el backend empiece a esperar.
        public async Task<string?> EnviarYEsperarRespuestaAsync(
            string idControlador,
            string mensaje)
        {
            Console.WriteLine("======================================");
            Console.WriteLine("[EnviarYEsperarRespuestaAsync]");
            Console.WriteLine($"ID: {idControlador}");
            Console.WriteLine($"Mensaje: {mensaje}");
            Console.WriteLine("======================================");
            ...
            _respuestasPendientes[idControlador] = tcs;

            try
            {
                // Primero dejamos preparada la espera.
                // Después enviamos al ESP32.
                ...
                // Aquí el POST queda esperando de forma asíncrona.
                var respuesta = await tcs.Task;
            ...
            {
                _respuestasPendientes.TryRemove(
                    idControlador,
                    out _
                );
            }
        ...
        {
            Console.WriteLine("======================================");
            Console.WriteLine("[GuardarMensajeRecibido]");
            Console.WriteLine($"ID: {idControlador}");
            Console.WriteLine($"Mensaje: {mensaje}");
            Console.WriteLine("======================================");

            // NUEVO:
            // Si existe un POST esperando una respuesta,
            // entregamos directamente este mensaje a ese POST.
            if (_respuestasPendientes.TryRemove(
                idControlador,
                out var pendiente))
            {
                Console.WriteLine(
                    "✅ Entregando respuesta al POST pendiente."
                );

                pendiente.TrySetResult(mensaje);
            ...
            // como respaldo.
            Console.WriteLine(
                "⚠️ No había POST esperando. Guardando mensaje."
            );
